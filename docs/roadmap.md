...
The repository root (`index.html`, `pages/`, `css/`, `js/`, `api/`, root `package.json`, `vercel.json`) currently serves two live merchant websites (Sérac and Fleur de Vie), both connected to the existing Firebase project `brandisby`. **No milestone in this roadmap may modify, delete, or redeploy those root-level files, or read/write Firestore/Storage in that project**, until a deliberate, explicitly-approved migration/cutover decision is made — separate from and after this roadmap's launch checklist (M6.3). The new app's database is entirely separate (Postgres, from M0.2, its own provider credentials). The one deliberate exception is Firebase **Authentication** on that same project, reused for identity only starting M0.3 — see `docs/decisions/002-firebase-auth-instead-of-clerk.md` for why that's safe and what guards it (Auth-only SDK imports, additive-only project config, never Firestore/Storage). Beyond that one exception, this should never require touching the legacy paths or data; if any future milestone appears to need to, stop and flag it rather than proceeding.
...
**M0.3 — Internal identity & auth** ✅ scaffolded (real credentials not yet provisioned)
Firebase Authentication (the existing `brandisby` project, reused for identity only — see `docs/decisions/002-firebase-auth-instead-of-clerk.md` for why this replaces the brief's Clerk example) behind an `AuthProvider` interface (`apps/web/src/server/auth/auth-provider.ts`) and a `FirebaseAuthProvider` adapter that imports only `firebase-admin/auth` — never Firestore/Storage. `userService.syncFromAuthProvider()` upserts the internal `User` row keyed by Brandisby's UUID, never a Firebase UID, closing audit §4/§14. Proven end-to-end by a Vitest integration test against the real Firebase **Auth Emulator** (`demo-brandisby`, fully offline — `firebase.json`/`.firebaserc`) plus real Postgres; `npm run test` now wraps `vitest run` in `firebase emulators:exec`. **Still needed:** real (non-emulator) Firebase Admin credentials — a service account scoped to Firebase Authentication Admin only on the existing project, not yet created; sign-in/sign-up UI itself (comes with M1.1's business-creation flow).

**M0.4 — Design tokens & base component library** ✅ done
Extended M0.1's design tokens with the aliases the Radix primitives need (`card`, `muted`, `destructive`, `input`, `ring`) plus a `radius-sm/md/lg/full` scale, all still pointing at the same brand source tokens. Base components hand-built on Radix UI primitives + `class-variance-authority` + `tailwind-merge` (the shadcn/ui *technique*, not the CLI — `ui.shadcn.com` is blocked by this environment's egress policy, so the registry-fetching `init`/`add` commands don't work here; the components are equally source-owned either way) under `apps/web/src/components/`: `ui/` for primitives (Button, Input, Label, Select, Card, Dialog, Table, Badge — Badge carries the success/warning/error/info status variants, never brand brown/beige, per brief §98), `shared/` for PageHeader and EmptyState, `forms/` for FormField (presentational, pairs with plain state or react-hook-form later) and FileUpload (presentational, no upload wiring until M4.1's StorageProvider). Verified by an internal-only showcase route (`/dev/ui`) rendered and screenshotted in a real browser — buttons, badges, form controls, select dropdown, dialog, table, and empty state all checked visually, not just typechecked. `packages/ui` deferred with the rest of `packages/*` per ADR 001.
...
- M0.3: AuthProvider abstraction/Firebase adapter/user sync/tests done against the Auth emulator; a real Firebase Authentication Admin-scoped service account on the existing project still needs to be provisioned (see "Next step" in the root README).
- M0.4: done — base component library at `apps/web/src/components/`, visually verified via `/dev/ui`.
- Phase 0 complete. M1.1 (business creation + Brandisby handle) is next.
