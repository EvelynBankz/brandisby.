...
**M3 — Brands + categories + profile pages**
`/brands/[slug]` profile pages (name, logo, cover image, tagline, category, location, about, story, founder, website/shop/social links, related articles). `/discover` browse page (search, category, location, featured, newest filters — not overbuilt). `/categories/[slug]` pages.
